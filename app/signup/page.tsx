...
import { UserPlus, Eye, EyeOff, AtSign, Lock, Mail, User, LogIn } from 'lucide-react';
...
function SignupPageContent() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      toast.success(`Welcome to ZiChat, ${data.user.name}!`);
      router.push('/chat');
    } catch (err: unknown) {
    ...
    <div className="min-h-screen flex items-center justify-center px-3 sm:px-4 bg-gradient-to-br from-background via-background to-primary/5 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-slide-up my-8">
        ...
        <div className="text-center mb-4 sm:mb-6">
          ...
          <p className="text-xs text-muted-foreground mt-1">Create your ZiChat account to start messaging</p>
        </div>
        ...
        <div className="bg-card rounded-xl sm:rounded-2xl p-5 sm:p-8 shadow-2xl border border-border">
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            {/* Username / ZiName */}
            <div className="space-y-1">
              <label htmlFor="username" className="text-xs font-medium text-foreground flex items-center gap-1.5">
                <AtSign className="h-3.5 w-3.5 text-muted-foreground" /> Username (ZiName)
              </label>
              <input
                id="username"
                type="text"
            ...
            <button
              type="submit"
              disabled={loading || googleLoading}
              className="w-full h-11 rounded-xl bg-primary text-primary-foreground font-semibold shadow-lg hover:opacity-90 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer mt-3"
            >
              {loading ? (
